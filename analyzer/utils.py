...
def setup_version_directory(base_dir, version):
    ...
    version_dir = os.path.join(base_dir, 'versions', f"{version}_{timestamp}")
...
def generate_report(parsed_flowmon_csv, plots_dir, report_file):
    """
    ...
    avg_throughput = df.filter(like='Throughput(Kbps)').mean().mean()
    avg_latency = df['AvgDelay(ms)'].mean()
